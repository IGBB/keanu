...
with open(args.input) as blast_taxon_coverage_file:
    annotated_lineage = ["root", "superkingdom", "kingdom", "phylum", "class", "order", "suborder", "family", "genus", "species"][::-1]
    blank = blast_taxon_coverage_file.readline()
    for line in blast_taxon_coverage_file:
        ...
        if len(parent_counts) > 0:
            highest = max(parent_counts.values())
            common_candidates = [k for k,v in parent_counts.items() if v == highest]
            for candidate, position in itertools.product(common_candidates, annotated_lineage):
                if tree.vertices[candidate].rank == position:
                    taxon = tree.vertices[candidate]
                    break
